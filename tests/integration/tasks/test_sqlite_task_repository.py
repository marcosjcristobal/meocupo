...
def test_set_task_deadline_persists_planning_through_sqlite() -> None:
    """Verify that deadline assignment survives a SQLite round trip."""

    # Arrange: define one initial calendar commitment.
    connection = sqlite3.connect(":memory:")
    deadline = TaskDeadline(
        due_on=date(2026, 8, 30),
    )
    ...
    try:
        ...
        repository = SqliteTaskRepository(connection=connection)
        create_task = CreateTask(repository=repository)
        set_task_deadline = SetTaskDeadline(repository=repository)
        get_task = GetTask(repository=repository)

        # Create the task without an initial deadline.
        created_task = create_task.execute(
            title="Renew the insurance.",
        )

        # Act: retrieve, assign, and save through SetTaskDeadline.
        updated_task = set_task_deadline.execute(
            task_id=created_task.id,
            deadline=deadline,
        )
    ...
    # Assert: the deadline survived SQLite persistence.
    assert updated_task.deadline == deadline
    assert retrieved_task.deadline == deadline

    # Assert: initial assignment is not historical postponement.
