"""Unit tests for the task-pausing application use case."""
...
def test_pause_task_transitions_and_persists_entity() -> None:
    """Verify that pausing coordinates retrieval, domain, and persistence."""

    # Arrange: only active work may transition into the paused state.
    task = Task(title="Study Docker.")
    task.start()
    repository = RecordingTaskRepository(task=task)
    use_case = PauseTask(repository=repository)

    # Act: request the lifecycle transition by portable identity.
    paused_task = use_case.execute(task_id=task.id)

    # Assert: the domain entity performed the transition.
    assert paused_task is task
...
def test_pause_task_rejects_non_uuid_identifiers(
...
def test_pause_task_reports_missing_identity() -> None:
    """Ensure that an absent task cannot be paused."""
...
def test_pause_task_does_not_save_rejected_transition() -> None:
    """Ensure that domain rejection leaves persistence untouched."""

    # Arrange: pending work cannot be paused.
    task = Task(title="Study Docker.")
    repository = RecordingTaskRepository(task=task)
    use_case = PauseTask(repository=repository)
